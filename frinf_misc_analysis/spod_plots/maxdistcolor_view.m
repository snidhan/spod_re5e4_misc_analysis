function [rgb,ucs,cnt,RGB,UCS,opts] = maxdistcolor_view(N,fun,varargin)
...
%
% See also MAXDISTCOLOR SRGB_TO_JAB SRGB_TO_LAB COLORNAMES COLORMAP RGBPLOT AXES SET LINES PLOT
...
[bgr,ucs,cnt,RGB,UCS,opts] = maxdistcolor(N,fun,varargin{:});
inw = true;
...
sRng = [   0,1;    0,1;    0,1;    0,1;   1,16;   1,16;   1,16;   0,360];
...
%
pAxH(9) = axes('Parent',uipanel(pnp{:}), axp{:},4);
pAxS(9) = struct('title','RGB Gamut (slow)',...
	'X',xyz{3}, 'Y',xyz{2}, 'Z',xyz{1}, 'fun',@fGamutTry);
%
pAxH(8) = axes('Parent',uipanel(pnp{:}), axp{:},3);
pAxS(8) = struct('title','Colors with Sort Path',...
	'X',xyz{3}, 'Y',xyz{2}, 'Z',xyz{1}, 'fun',@fSortPath);

pAxH(7) = axes('Parent',uipanel(pnp{:}), axp{:},3);
pAxS(7) = struct('title','Colors with RGB Cube',...
	'X',xyz{3}, 'Y',xyz{2}, 'Z',xyz{1}, 'fun',@fCubeRGB);
%
...
%% Main Plot Functions %%
%
	function fGamutTry(~) % ALPHASHAPE is slow, run only on demand.
	...
	function fGamutRGB(axh) % Show the RGB gamut.
	...
	function fScatterMat(axh) % Matrix scatter plot.
		...
		for rr = 3:-1:1
			for cc = 3:-1:1
				...
				if rr==cc
					text(0.5,0.5, xyz{rr}, 'Parent',pScH(rr,cc),...
						'FontWeight','bold', 'Color',fgd)
				else
			...
			switch aud
				...
				case 3
					scatter3(idk*ones(1,N), 1:N, dst, 123, bgr,...
						sgc{:}, 'MarkerFaceColor',bgr(idk,:), 'Parent',axh);
				otherwise
					error('Sorry, I can''t plot %d dimensions.',aud)
			end
			hold(axh,'on')
		end
		%
...
%
	...
	function fUpDtMap(~,~) % Update colormap using options structure.
		if eUiX
			...
			set(figH, 'Pointer','watch')
			drawnow()
			[bgr,ucs,cnt,RGB,UCS] = maxdistcolor(N,fun,opts);
			set(figH, 'Pointer',figY)
			inw = true;
			ohm = pow2([opts.bitR,opts.bitG,opts.bitB])-1;
			% Update colorbar:
