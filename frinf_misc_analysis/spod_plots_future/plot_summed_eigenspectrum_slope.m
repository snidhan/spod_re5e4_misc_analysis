...
summed_spod_mode = sum(eigvalues_spectrum_plot(2).eigvalue(:,:),2);
...
idx = f>0;
plot(f(idx), summed_spod_mode(idx), 'k-', 'Linewidth', 2)
...
f_fit = f(ix1:ix2);
eigvalue_fit = summed_spod_mode(ix1:ix2,1);

log_eigvalue_fit = log(eigvalue_fit);
log_f_fit =  log(f_fit);
[coeffs, S] = polyfit(log_f_fit, log_eigvalue_fit, 1)

hold on;
loglog(f_fit, 5*10^-5*f_fit.^coeffs(1), 'k--', 'Linewidth',2);
text(1.5, 5*10^-5,'$\mbox{\textit{St}}^{-2.39}$','interpreter','latex','FontSize', 20);
...
summed_spod_mode = sum(eigvalues_spectrum_plot(5).eigvalue(:,:),2);
...
hold on;
loglog(f_fit, 5*10^-6*f_fit.^coeffs(1), 'k--', 'Linewidth',2);
text(1.5, 10^-5,'$\mbox{\textit{St}}^{-1.98}$','interpreter','latex','FontSize', 20);
...
summed_spod_mode = sum(eigvalues_spectrum_plot(3).eigvalue(:,:),2);

hold on;
box on;

f = eigvalues_spectrum_plot(3).freq';
idx = f>0;
plot(f(idx), summed_spod_mode(idx), 'k-', 'Linewidth', 2)
...
hold on;
loglog(f_fit, 5*10^-5*f_fit.^coeffs(1), 'k--', 'Linewidth',2);
text(1.5, 5*10^-5,'$\mbox{\textit{St}}^{-2.29}$','interpreter','latex','FontSize', 20);
...
summed_spod_mode = sum(eigvalues_spectrum_plot(6).eigvalue(:,:),2);

hold on;
box on;

f = eigvalues_spectrum_plot(6).freq';
idx = f>0;
plot(f(idx), summed_spod_mode(idx), 'k-', 'Linewidth', 2)
...
log_eigvalue_fit = log(eigvalue_fit);
log_f_fit =  log(f_fit);
[coeffs, S] = polyfit(log_f_fit, log_eigvalue_fit, 1) %#ok<*NOPTS>

hold on;
loglog(f_fit, 5*10^-6*f_fit.^coeffs(1), 'k--', 'Linewidth',2);
text(1.5, 10^-5,'$\mbox{\textit{St}}^{-1.97}$','interpreter','latex','FontSize', 20);
...
%% Saving the plot
% set(gcf, 'PaperPositionMode', 'auto');
print(gcf,strcat(dirout, 'eigenspectrum_m12_x_D_20_80_allmodessummed_withslopes.png'),'-dpng','-r600');  
print(gcf,strcat(dirout, 'eigenspectrum_m12_x_D_20_80_allmodessummed_withslopes.eps'),'-depsc','-r600');
