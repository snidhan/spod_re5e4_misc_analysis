...
loc_planes = [70; 75; 80; 85; 90; 95; 100; 110; 120];
nstart = 1892600;
nend   = 2613200;
stride  = 100;
...
%% Plot the unscaled reynolds stresses

figure;
hold on;

C = {'-','-','-','-','--','--','--','--', '--'}; % Cell array of linestyle
color = {'c','b','r','g','k','g','r','b','c'}; % Cell array of linestyle

reystress_uw_1d_smooth = smoothdata(reystress_uw_1d,'loess');

for i = 1:size(loc_planes,1)
    plot([rc/LK_TKE_loc_planes(i,2)], [-reystress_uw_1d(:,i)/TKE_centerline_loc_planes(i,2)], ...
        'Linestyle', C{i}, 'color', color{i}, 'Linewidth',2);
      %  plot([rc/LK_TKE_loc_planes(i,2)], [-reystress_uw_1d_smooth(:,i)/max(abs(reystress_uw_1d_smooth(:,i)))],'o', 'color', color{i});
end
...
% print(gcf,strcat('./', 'rxr_normalized_uinfty_ud_dld_x_D_70_120.png'),'-dpng2','-r600');  
% print(gcf,strcat('./', 'rxr_normalized_uinfty_ud_dld_x_D_70_120.eps'),'-depsc2','-r600');

set(gcf, 'PaperPositionMode', 'auto');
print(gcf,strcat('./', 'rxr_normalized_ko_x_D_70_120.png'),'-dpng2','-r600');  
print(gcf,strcat('./', 'rxr_normalized_ko_x_D_70_120.eps'),'-depsc2','-r600');
...
log_max_rs = log(max_tke');
[coeffs_tke_max, S] = polyfit(log_x_D, log_max_rs, 1)
%% Fit a function to single profile at x/D = 90 normalizing by the max(RS)

%close all;

% n = 5;
% yr = -reystress_uw_1d_smooth(:,n)/max(abs(reystress_uw_1d_smooth(:,n)));
% %yr = -reystress_uw_1d_smooth(:,n)/TKE_centerline_loc_planes(n,2);
% eta = rc/LK_mean_loc_planes(n,2);
% 
% figure;
...
%% Fit a function to all the profiles from x/D = 70 to x/D = 120 by normalizing by max(RS)
...
%% Fit a function to all the profiles from x/D = 70 to x/D = 120 by normalizing by K_centerline
...
etotal = [];
for i = 1:size(loc_planes,1)
    yr = -reystress_uw_1d_smooth(:,i)/max(abs(reystress_uw_1d(:,i)));
    eta = rc/LK_mean_loc_planes(i,2);
    yr_zero_padded = [0; yr];
    eta_zero_padded = [0; eta];
    ytotal = [ytotal; yr_zero_padded];
...
max_tke_ld = max(abs(tke_1d_smooth))'./LK_TKE_loc_planes(:,2);
...
h2 = loglog(LK_mean_loc_planes(1:end,1),y1, 'r--', 'Linewidth', 2);
